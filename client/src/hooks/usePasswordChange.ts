...
async function executePasswordChange(data: ChangePasswordPayload) {
  return await changePasswordService({ old_password: data.old_password, new_password: data.new_password });
}
...
export function usePasswordChange() {
  ...
  const onSubmit = async (data: ChangePasswordPayload) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : APP_MESSAGES.COMMON.ERROR;
