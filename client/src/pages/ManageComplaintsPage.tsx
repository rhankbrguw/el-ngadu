...
const ComplaintListContent = ({ pengaduan, isDesktop }: ComplaintListContentProps) => {
  const list = Array.isArray(pengaduan) ? pengaduan : [];
  if (list.length === 0) {
    return (
  ...
  if (isDesktop) return <ComplaintTable pengaduanList={list} />;
  return <div className="p-3"><ComplaintCards pengaduanList={list} /></div>;
};
