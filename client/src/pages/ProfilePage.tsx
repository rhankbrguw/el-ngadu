...
import type { Masyarakat, Petugas } from "@/types";
...
const ProfileHeader = ({ user, isMasyarakat }: ProfileHeaderProps) => {
  const isPetugasAdmin = user.userType === "petugas" && (user as Petugas).level === "admin";
  const roleVariant = isPetugasAdmin ? "default" : user.userType === "petugas" ? "secondary" : "outline";
  const roleText = isPetugasAdmin ? "Administrator" : user.userType === "petugas" ? "Petugas" : "Masyarakat";
  const displayName = isMasyarakat ? (user as Masyarakat).nama : (user as Petugas).nama_petugas;
...
const ProfileSecurityTab = () => (
  <TabsContent value="security" className="m-0 space-y-4 focus-visible:outline-none focus-visible:ring-0">
    <Card>
      ...
      <CardContent><ProfileEditForm /></CardContent>
    </Card>
    <Card>
      <CardHeader>
        <CardTitle>{APP_MESSAGES.PROFILE.EDIT_PASSWORD}</CardTitle>
        <CardDescription>Ganti kata sandi secara berkala untuk menjaga keamanan akun Anda.</CardDescription>
      </CardHeader>
...
export default function ProfilePage() {
  ...
  if (!user) {
    return <div className="flex justify-center p-12"><p>Data pengguna tidak ditemukan.</p></div>;
  }
  ...
  return (
    <div className="space-y-6">
      <ProfileHeader user={user} isMasyarakat={isMasyarakat} />
      <Tabs defaultValue="info" className="w-full">
        <TabsList className="grid w-full grid-cols-2 sm:w-form">
          <TabsTrigger value="info" className="gap-2"><Info className="h-4 w-4 hidden sm:block" /> Profil</TabsTrigger>
          <TabsTrigger value="security" className="gap-2"><Shield className="h-4 w-4 hidden sm:block" /> Keamanan</TabsTrigger>
        </TabsList>
        <div className="mt-6">
