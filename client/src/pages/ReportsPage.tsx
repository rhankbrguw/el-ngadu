...
const ReportPageHeader = ({ filterStatus, setFilterStatus, filteredData }: ReportHeaderProps) => (
  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
    <div className="flex items-center gap-4">
      <div className="bg-primary/10 p-2 rounded-lg"><FileSpreadsheet className="h-6 w-6 text-primary flex-shrink-0" /></div>
    ...
    <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
      <Select value={filterStatus} onValueChange={setFilterStatus}>
        <SelectTrigger className="w-full sm:w-report-select"><SelectValue placeholder={APP_MESSAGES.SEARCH.FILTER_STATUS} /></SelectTrigger>
        <SelectContent>
      ...
      <Button onClick={() => handleExportCSV(filteredData)} variant="outline" disabled={filteredData.length === 0} className="w-full sm:w-auto">
        <Download className="mr-2 h-4 w-4" /><span className="hidden sm:inline">{REPORT_STRINGS.BTN_CSV}</span><span className="sm:hidden">CSV</span>
      </Button>
      <Button onClick={() => handleExportPDF(filteredData)} disabled={filteredData.length === 0} className="w-full sm:w-auto">
        <FileText className="mr-2 h-4 w-4" /><span className="hidden sm:inline">{REPORT_STRINGS.BTN_PDF}</span><span className="sm:hidden">PDF</span>
      </Button>
    </div>
  </div>
);
