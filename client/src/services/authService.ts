...
import type { RegisterPayload } from "@/lib/validators/auth";
import type { ProfileEditPayload, ChangePasswordPayload } from "@/lib/validators/profile";
...
const mapUserData = (userData: Record<string, unknown>): User => {
  if (userData.nik) {
    return { ...(userData as unknown as Masyarakat), userType: "masyarakat" };
  }
  if (userData.id_petugas) {
    return { ...(userData as unknown as Petugas), userType: "petugas" };
  }
