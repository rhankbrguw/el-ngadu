...
class OfficerController {
    ...
    private function validateUpdateInput(array $input, int $id): array {
        $validation = \Core\AppValidator::make($input, [
            'nama_petugas' => 'nullable|valid_name',
            'username'     => "nullable|min:3|max:30|unique_global:petugas,{$id}",
            'password'     => 'nullable|min:8',
            'telp'         => "nullable|valid_phone|unique_global:petugas,{$id}",
            'level'        => 'nullable|in:admin,petugas',
            'email'        => "nullable|email|trusted_email|unique_global:petugas,{$id}"
        ]);
