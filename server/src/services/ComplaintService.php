...
class ComplaintService {
    ...
    public function createComplaint(array $data, ?array $file, string $userId, string $userName): int {
        ...
        try {
            ...
            $isAnonim = !empty($data['is_anonim']) && $data['is_anonim'] != '0';
            $displayName = $isAnonim ? 'Masyarakat (Anonim)' : $userName;
            $this->notifyOfficers($this->repository->getPdo(), $id, $data['judul'], $data['kategori'], $displayName);
