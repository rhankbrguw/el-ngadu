...
class ComplaintReadService {
    ...
    public function getAll(int $page, int $limit, ?string $status = null, ?string $kecamatan = null, ?string $q = null): array {
        $offset = ($page - 1) * $limit;
        $total = $this->repository->getCount($status, $kecamatan, $q);
        $data = $this->repository->getAll($limit, $offset, $status, $kecamatan, $q);
        return [
            'pagination' => ['current_page' => $page, 'total_pages' => (int)ceil($total / $limit), 'total_records' => $total, 'limit' => $limit],
            'data' => $data,
            'available_kecamatan' => $this->repository->getDistinctKecamatan()
        ];
    }
