...
class Config {
    ...
    public const SMTP_DEFAULT_HOST = 'smtp.resend.com';
    public const SMTP_DEFAULT_PORT = 587;
    public const SMTP_DEFAULT_FROM = 'no-reply@rhankbrguw.xyz';
    ...
    public const DEFAULT_PROD_APP_URL = 'https://el-ngadu.rhankbrguw.xyz';

    public static function getAppUrl(): string {
        $envUrl = $_ENV['APP_URL'] ?? getenv('APP_URL');
        if (!self::isDev()) {
            return !empty($envUrl) && !str_contains($envUrl, 'localhost') ? $envUrl : self::DEFAULT_PROD_APP_URL;
        }
    ...
    public static function getSmtpConfig(): array {
        $resendApiKey = self::getResendApiKey();
        $isResend = !empty($resendApiKey);

        return [
            'host' => $_ENV['SMTP_HOST'] ?? getenv('SMTP_HOST') ?: ($isResend ? 'smtp.resend.com' : self::SMTP_DEFAULT_HOST),
            'port' => (int)($_ENV['SMTP_PORT'] ?? getenv('SMTP_PORT') ?: self::SMTP_DEFAULT_PORT),
            'secure' => $_ENV['SMTP_SECURE'] ?? getenv('SMTP_SECURE') ?: 'tls',
            'user' => $_ENV['SMTP_USER'] ?? getenv('SMTP_USER') ?: ($isResend ? 'resend' : ''),
            'pass' => $_ENV['SMTP_PASS'] ?? getenv('SMTP_PASS') ?: $resendApiKey,
            'from' => $_ENV['RESEND_FROM'] ?? $_ENV['SMTP_FROM'] ?? getenv('SMTP_FROM') ?: self::SMTP_DEFAULT_FROM,
            'from_name' => $_ENV['RESEND_FROM_NAME'] ?? $_ENV['SMTP_FROM_NAME'] ?? getenv('SMTP_FROM_NAME') ?: self::SMTP_DEFAULT_FROM_NAME,
        ];
    }
}
