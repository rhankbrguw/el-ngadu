...
class OfficerRepository {
    ...
    public function getPaginated(int $limit, int $offset): array {
        try {
            ...
            throw new \Core\InternalException(\Constants\AppMessages::ERR_DB_FETCH_PETUGAS . $e->getMessage());
        }
    }
    ...
    public function delete(int $id): int {
        try {
            ...
            throw new \Core\InternalException(\Constants\AppMessages::ERR_DB_DELETE_PETUGAS . $e->getMessage());
        }
    }

    public function search(string $searchTerm): array {
        try {
            $sql = "SELECT id_petugas, nama_petugas, username, telp, level, email FROM petugas WHERE nama_petugas LIKE ? OR username LIKE ? ORDER BY nama_petugas ASC LIMIT 50";
            $stmt = $this->pdo->prepare($sql);
