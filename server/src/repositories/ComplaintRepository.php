...
class ComplaintRepository {
    ...
    private function buildFilterClause(?string $status, ?string $kecamatan, ?string $q, array &$params): string {
        ...
        if ($q && trim($q) !== '') {
            $sql .= " AND (p.judul LIKE ? OR p.isi LIKE ? OR p.lokasi LIKE ? OR p.kecamatan LIKE ? OR (CASE WHEN p.is_anonim = 1 THEN '' ELSE m.nama END LIKE ?))";
            $term = "%" . trim($q) . "%";
    ...
    public function getAll(int $limit, int $offset, ?string $status = null, ?string $kecamatan = null, ?string $q = null): array {
        $params = [];
        $where = $this->buildFilterClause($status, $kecamatan, $q, $params);
        $sql = "SELECT p.id, p.judul, p.status, p.kategori, p.prioritas, p.kecamatan, p.kelurahan, p.lokasi, p.created_at, p.is_anonim,
    ...
    public function search(string $q, ?string $userNik): array {
        $sql = "SELECT p.id, p.judul, p.status, p.kategori, p.prioritas, p.kecamatan, p.kelurahan, p.created_at, p.is_anonim,
                CASE WHEN p.is_anonim = 1 THEN 'Masyarakat (Anonim)' ELSE m.nama END AS nama_pelapor
                FROM pengaduan p JOIN masyarakat m ON p.nik_masyarakat = m.nik 
                WHERE (p.judul LIKE ? OR p.isi LIKE ? OR p.kategori LIKE ? OR p.lokasi LIKE ? OR p.kecamatan LIKE ? OR p.kelurahan LIKE ? OR (CASE WHEN p.is_anonim = 1 THEN '' ELSE m.nama END LIKE ?))";
        $term = "%$q%";
    ...
    public function createComplaint(array $data, string $userId, ?string $fotoPath): int {
        ...
        $stmt->execute([
            ...
            (!empty($data['is_anonim']) && $data['is_anonim'] != '0') ? 1 : 0, $userId, $fotoPath
        ]);
