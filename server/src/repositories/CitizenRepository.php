...
class CitizenRepository {
    ...
    public function createAdmin(array $data): int {
        try {
            ...
            if ($e->getCode() === '23000') {
                throw new \Core\ConflictException(\Constants\AppMessages::ERR_NIK_ATAU_USERNAME_SUDAH_TERDAFTAR);
            }
            throw new \Core\InternalException(\Constants\AppMessages::ERR_DB_SAVE . ': ' . $e->getMessage());
        }
    }
    ...
    public function getCount(): int {
        try {
            ...
            throw new \Core\InternalException(\Constants\AppMessages::ERR_DB_FETCH_MASYARAKAT . $e->getMessage());
        }
    }
    ...
    public function delete(string $nik): int {
        try {
            ...
            throw new \Core\InternalException(\Constants\AppMessages::ERR_DB_DELETE_MASYARAKAT . $e->getMessage());
        }
    }

    public function search(string $searchTerm): array {
        try {
            $sql = "SELECT nik, nama, username, telp, created_at FROM masyarakat WHERE nama LIKE ? OR username LIKE ? OR nik LIKE ? ORDER BY nama ASC LIMIT 50";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute([$searchTerm, $searchTerm, $searchTerm]);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (\PDOException $e) {
            throw new \Core\InternalException(\Constants\AppMessages::ERR_SEARCH_FAILED . $e->getMessage());
        }
    }
}
